...
class TestSeacrhLocators:
    ...
    LOCATOR_CONTACT_MESSAGE = (By.XPATH, """//*[@id="contact"]/div[3]/label/span/textarea""")
    LOCATOR_CONTACT_BUTTON = (By.XPATH, """//*[@id="contact"]/div[4]/button/span""")
...
class OperationsHelper(BasePage):
    ...
    def click_contact_link(self):
        ...
        self.find_element(TestSeacrhLocators.LOCATOR_CONTACT_LINK).click()

    def add_contact_name(self, name):
        time.sleep(2)
        logging.info(f"Send '{name}' to element {TestSeacrhLocators.LOCATOR_CONTACT_NAME[1]}")
        title_field = self.find_element(TestSeacrhLocators.LOCATOR_CONTACT_NAME)
        title_field.clear()
    ...
    def add_contact_email(self, email):
        ...
        title_field = self.find_element(TestSeacrhLocators.LOCATOR_CONTACT_EMAIL)
        title_field.clear()
        title_field.send_keys(email)

    def add_contact_message(self, message):
        time.sleep(2)
        logging.info(f"Send '{message}' to element {TestSeacrhLocators.LOCATOR_CONTACT_MESSAGE[1]}")
        content_field = self.find_element(TestSeacrhLocators.LOCATOR_CONTACT_MESSAGE)
        content_field.clear()
        content_field.send_keys(message)
